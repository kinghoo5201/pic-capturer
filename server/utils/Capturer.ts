...
export default class Capturer {
    ...
    /**截图 */
    public async screenShoot(url: string, pageOptions?: puppeteer.PageFnOptions, screenShootOpt?: puppeteer.ScreenshotOptions) {
        if (!this.page) {
            return false;
        }
        await this.page.goto(url, pageOptions ? pageOptions : {});
        await this.page.screenshot(screenShootOpt ? screenShootOpt : {})
        return true;
    }
    /**更新配置 */
    ...
    private async initBrowser() {
        this.browser = await puppeteer.launch(this.launchOpt ? {
            ...this.launchOpt
        } : {});
